...
Namespace AniDB
    ...
    Public Class AniDBClient
        ...
        Public Const DataCommandsVersion As String = "0.0.0.1-Beta.4"
        Public Const SearchCommandsVersion As String = "0.0.0.1-Beta.2"
        Public Const HelpersVersion As String = "0.0.0.1-Beta.4"
        ...
        ''' <summary>
        ''' Occures when IsClientValid changes value
        ''' </summary>
        ''' <param name="Value">True = Valid, False = Invalid</param>
        Public Event OnClientValidityChanged(Value As Boolean)
...
#End Region
        Private Property _IsClientValid As Boolean = False
        ''' <summary>
        ''' If returns False all the get functions will return nothing !,Call ValidClient in order to recheck client validity
        ''' </summary>
        ''' <returns></returns>
        Public ReadOnly Property IsClientValid As Boolean
            Get
                Return _IsClientValid
            End Get
        End Property
        ''' <summary>
        ''' Initialize a new AniDBClient, Call ValidClient after initialization
        ''' </summary>
        ...
        End Sub
        Public Async Function ValidClient() As Task(Of Boolean)
            Dim Validity = Await SharedFunctions.CheckClient(Client, ClientVer)
            If Validity <> IsClientValid Then
                RaiseEvent OnClientValidityChanged(Validity)
            End If
            _IsClientValid = Validity
...
#Region "Data Commands"
        ...
        Public Async Function Anime(aid As Integer, Optional PreloadPicture As Boolean = True, Optional FetchCharacters As Boolean = True, Optional PreloadCharacterPicture As Boolean = True) As Task(Of AnimeElement)
            ...
            If Not IsClientValid Then Return Nothing
